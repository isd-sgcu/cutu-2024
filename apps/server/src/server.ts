...
export async function initServer(app: Express, server: HTTPServer) {
  ...
  const playerController = new PlayerController(playerIO, new PlayerService(gameRepository));
  playerIO.on("connection", playerController.onConnection.bind(playerController));
  ...
  const adminController = new AdminController(adminIO, new AdminService(gameRepository));
